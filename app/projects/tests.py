...
class ProjectTestCase(TestCase):
    ...
    def setUp(self):
        user = get_user_model().objects.create(username='test01', email='test@test.fr', password="test")
        category = Category.objects.create(label="Test",abbreviated="test")
        Project.objects.create(user=user, name="test", summarize="a resume", description="a desc", price=20000, category=category, slug="blag")
